    ...
    <div className={`${className !== undefined ? className : ""} flex justify-center card`}>
      <Link href={{ pathname: pathname, query: query }} passHref={passHref}>
        <a className="flex flex-col items-center text-xl font-text w-72">
          <div className="relative flex items-center justify-center cursor-pointer h-72 w-72 bg-secondary rounded-xl">{img && <Image src={ASSETS_URL + img} alt="Descriptive image for an event" layout="fill" objectFit="cover" className="rounded-xl" />}</div>
